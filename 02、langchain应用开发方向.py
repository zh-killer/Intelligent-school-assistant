...
agent = create_agent(
    model=llm,
    tools=[get_weather, search_baike]
)
...
def process_user_input(user_input, chat_history):
    ...
    try:
        ...
        # 检查是否调用了工具，添加额外信息
        if "get_weather" in str(result) or "天气" in user_input:
            pass  # 天气信息已经由工具格式化好了
        elif "search_baike" in str(result) or "百科" in user_input or "是什么" in user_input:
            pass  # 百科信息已经由工具格式化好了
        else:
            # 通用对话，使用智谱API增强回复
            try:
